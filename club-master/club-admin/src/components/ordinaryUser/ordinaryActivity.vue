<template>
  <div v-if="activityPage != null">
    ...
    <el-table :data="activityData"
              stripe style="width:100%"  border size="mini" >
      <el-table-column prop="id" label="活动id" width="150"></el-table-column>
      <el-table-column prop="activnpityName" label="活动名称" width="260" :sortable="true"></el-table-column>
      <el-table-column prop="site" label="活动地点" width="240" :sortable="true"></el-table-column>
      <el-table-column prop="holdTime" label="活动举行时间" width="280" :sortable="true"></el-table-column>
      <el-table-column  label="报名参加">
        <template slot-scope="scope">
          <el-button type="primary" icon="el-icon-plus" @click="joinActivity(scope.row)">报名</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      joinActivity: function(row) {
        var id = row.id;
        console.log(row.id);
        this.$router.push({ name: 'ordinaryJoinActivity',query: { id: id } });
      },
